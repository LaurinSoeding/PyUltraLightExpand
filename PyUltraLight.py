...
def prog_bar(iteration_number, progress, tinterval):
    ...
    #text2 = '{}{}'.format( '\nEstimated time remaining is ', display_time( (iteration_number-progress)*tinterval, 5 ) )
...
def calculate_energies(
    ...
    #Saves physical quantities (hopefully)
        if (save_options[3]):
            ...
            egyarr = ne.evaluate('real((abs(psi))**2/(scalefactor**3))') #compute physical density
            egyarr = ne.evaluate('real((-cmass/(distarray*scalefactor))*egyarr)') #compute physical energy density
            egpcmlist.append(Vcell * scalefactor**3 * np.sum(egyarr)) #add total gravitational energy to the list
            tot = Vcell * scalefactor**3 * np.sum(egyarr) #add total gravitational energy to the total energy

            # Gravitational potential energy density of self-interaction of the condensate
            egyarr = ne.evaluate('real(0.5*(phisp+(cmass)/(distarray*scalefactor))*real((abs(psi))**2)/scalefactor**3)')
            egpsilist.append(Vcell * scalefactor**3 * np.sum(egyarr))
            tot = tot + Vcell * scalefactor**3 * np.sum(egyarr)
            ...
            egyarr = ne.evaluate('real( (-0.5*conj(psi)*funct)/scalefactor**5 )')# power 3 from the wavefunctions and power 2 from the derivatives
            ekandqlist.append(Vcell * scalefactor**3 * np.sum(egyarr))
            tot = tot + Vcell * scalefactor**3 * np.sum(egyarr)
...
def choose_timestep(scalefactor, gridlength, resol, step_factor, ):
    h = step_factor * scalefactor**2 * (gridlength/float(resol))**2/np.pi
    return h
...
def evolve(central_mass, num_threads, length, length_units, resol, initial_z, final_z, step_factor, skip_saves, save_options,
    ...
    tdummy = z_to_t(initial_z)
    num_timesteps = 0
    while(tdummy < z_to_t(final_z)):
        num_timesteps+=1
        tdummy += choose_timestep(t_to_a(tdummy), gridlength, resol, step_factor, )
    ...
    phisp = ne.evaluate("phisp-(cmass)/distarray/scalefactor")
    ...
    for ix in range(num_timesteps):
        ...
        future_scalefactor = t_to_a(timeparameter+h)
        psi = ne.evaluate("exp(-0.5*1j*h*phisp)*psi")
        funct = fft_psi(psi)
        funct = ne.evaluate("funct*exp(-1j*0.25*h*karray2*(1/scalefactor/scalefactor+1/future_scalefactor/future_scalefactor))")
        ...
        timeparameter+=h
        scalefactor = t_to_a(timeparameter)
        redshift = t_to_z(timeparameter)
